...
export const MockServerService = {
   ...
   */
  async start({ port, name, routes }: { port: number, name?: string, routes: MockRoute[] }) {
    const id = generateServerId();
    const sanitizedRoutes = (routes || []).map((r: MockRoute) => ({
      method: r.method || "GET",
      path: r.path || "/",
  ...
  async stop(id: string) {
    if ((window as any).api?.mockStop) {
      return (window as any).api.mockStop({ id });
    }
   ...
   */
  async updateRoutes(id: string, routes: MockRoute[]) {
    if ((window as any).api?.mockUpdateRoutes) {
      return (window as any).api.mockUpdateRoutes({ id, routes });
    }
    return { error: "Mock server API not available" };
  },
   ...
   */
  generateRoutesFromCollection(collection: any) {
