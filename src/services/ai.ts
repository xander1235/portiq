 ...
 */
async function aiSafeFetch(path: string, options: any = {}) {
...
async function callLLM(provider: string, model: string, apiKey: string, systemPrompt: string, userMessage: any, format = "json", sessionContext: any = null) {
  if (!apiKey) {
...
export async function generateRequestFromPrompt(
  ...
  if (semanticSearchEnabled) {
    try {
      ...
      if (semanticResults && semanticResults.length > 0) {
        ...
        const semanticReqObjects: any[] = [];
        semanticResults.forEach((sr: any) => {
          const found = flatReqs.find((r: any) => r.id === sr.id);
  ...
  const collectionContext = collections.map((c: any) => ({
    id: c.id,
    name: c.name,
  ...
  if (response.result.operations) {
    response.result.operations.forEach((op: any) => {
      if (op.type === "SUGGEST_ENDPOINTS" && op.payload && op.payload.endpointIds) {
        ...
        op.payload.endpointIds.forEach((id: any) => {
          const found = relevantRequests.find((r: any) => r.id === id);
