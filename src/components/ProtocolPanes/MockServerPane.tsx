 ...
 */
export interface MockServerPaneProps {
  collections: any[];
}

export function MockServerPane({ collections }: MockServerPaneProps) {
  const [servers, setServers] = useState<any[]>([]);
  const [port, setPort] = useState(3100);
  const [name, setName] = useState("");
  const [routes, setRoutes] = useState<any[]>([MockServerService.createDefaultRoute()]);
  ...
  const handleStop = useCallback(async (serverId: string) => {
    try {
      await MockServerService.stop(serverId);
      addLog("info", `Server ${serverId} stopped`);
      refreshServers();
    } catch (err: any) {
  ...
  const updateRoute = useCallback((idx: number, updates: any) => {
