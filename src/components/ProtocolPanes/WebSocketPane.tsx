...
export interface WebSocketPaneProps {
  url: string;
  setUrl: (v: string) => void;
  config: any;
  setConfig: React.Dispatch<React.SetStateAction<any>>;
  currentRequestId: string;
  updateRequestState: any;
  getEnvVars: () => Record<string, string>;
  interpolate: (v: string) => string;
  onStatusChange?: (status: string) => void;
  clearSignal: number;
  onResponseChange?: (resp: any) => void;
}
...
function rowsToProtocols(rows: any[]) {
  return (rows || [])
    .filter((row) => row.key && row.enabled !== false)
...
export function WebSocketPane({
  ...
  const [messages, setMessages] = useState<any[]>(() => Array.isArray(wsConfig.messages) ? wsConfig.messages : []);
  ...
  const updateWsConfig = useCallback((patch: any) => {
    setConfig((prev: any) => {
  ...
  const handleHeadersRowsChange = useCallback((nextRows: any[]) => {
    updateWsConfig({
      headersRows: nextRows as any[],
      headersText: JSON.stringify(rowsToObject(nextRows), null, 2)
    });
  }, [updateWsConfig]);

  const handleHeadersTextChange = useCallback((value: string) => {
    const patch: any = { headersText: value };
    try {
  ...
  const handleProtocolRowsChange = useCallback((nextRows: any[]) => {
    updateWsConfig({
    ...
    if ((!wsConfig.protocolRows || wsConfig.protocolRows.length === 0) && wsConfig.protocolsText) {
      updateWsConfig({
        protocolRows: wsConfig.protocolsText
          ...
          .filter(Boolean)
          .map((value: string) => ({ key: value, value: "", comment: "", enabled: true }))
      });
    }
  ...
  const appendMessage = useCallback((msg: any) => {
    setMessages((prev: any[]) => [...prev, msg].slice(-200));
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubStatus = manager.on("statusChange", (nextStatus: string) => {
      if (cancelled) return;
    ...
    const unsubError = manager.on("error", (data: any) => {
      if (cancelled) return;
  ...
  const handleConnect = useCallback(async () => {
    ...
    if (!validation.valid) {
      setError(validation.errors.join(", ") as any);
      return;
    }
  ...
  const sentCount = useMemo(
    () => (messages as any[]).filter((message: any) => message.direction === "outgoing").length,
    [messages]
  );
  const receivedCount = useMemo(
    () => (messages as any[]).filter((message: any) => message.direction === "incoming").length,
    [messages]
  );
