...
interface ExportModalProps {
    ...
    exportTargetNode: any;
    exportSelections: Set<string>;
    setExportSelections: (selections: Set<string>) => void;
    exportInterpolate: boolean;
    setExportInterpolate: (interpolate: boolean) => void;
    renderExportTree: (node: any) => React.ReactNode;
    getExportPayload: () => { jsonStr: string; fileName: string };
}
                ...
                <div className="export-header">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        <div style={{ display: 'flex', gap: '8px' }}>
                            ...
                            <button className="ghost input compact" style={{ fontSize: '11px', padding: '2px 8px' }} onClick={() => setExportSelections(new Set<string>())}>None</button>
                        </div>
                        <div style={{ fontSize: '0.8rem', color: 'var(--muted)' }}>
                            Requests <span style={{ color: 'var(--accent-green)', fontWeight: 'bold' }}>{Array.from(exportSelections).filter(id => (id as string).startsWith('req-')).length}</span>
                        </div>
                    </div>
                </div>
