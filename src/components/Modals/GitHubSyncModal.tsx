...
import { getGitHubToken, setGitHubToken, requestDeviceCode, pollForToken } from "../../services/githubAuth";
import { testGitHubConnection, pushStateToGitHub, pullStateFromGitHub, pushHistoryToGitHub, previewEnvironmentsForSync } from "../../services/githubSync";
...
interface SyncState {
    status: 'syncing' | 'synced' | 'error' | 'pulling' | 'pulled';
    label: string;
    detail: string;
}
...
interface GitHubSyncModalProps {
    ...
    onPulledState?: (state: any) => void;
}

export function GitHubSyncModal({ isOpen, onClose, onSyncStateChange, onPulledState }: GitHubSyncModalProps) {
    ...
    const [maskedVarIds, setMaskedVarIds] = useState<Set<string>>(new Set());
    ...
    const checkConnection = async (t: string) => {
    ...
    const handlePreparePush = () => {
        ...
        envs.forEach((env: any) => {
            (env.vars || []).forEach((v: any, i: number) => {
    ...
    const performActualPush = async (maskedIds: Set<string>) => {
        onSyncStateChange?.({
            status: "syncing",
            label: "Syncing...",
                    ...
                    {authStep === "preview" && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            ...
                            <div style={{ maxHeight: '250px', overflowY: 'auto', background: 'var(--panel-2)', borderRadius: '6px', border: '1px solid var(--border)' }}>
                                {previewData.filter(env => env.vars && env.vars.length > 0).map((env: any) => (
                                                ...
                                                <div key={vId} style={{ padding: '8px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', fontSize: '0.85rem' }}>
                                                    ...
                                                    <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', flexShrink: 0 }}>
                                                        <input
                                                            ...
                                                            checked={isMasked}
                                                            onChange={(e) => {
                                                                const newMasks = new Set<string>(maskedVarIds);
