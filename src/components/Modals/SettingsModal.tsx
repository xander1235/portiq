...
interface SettingsModalProps {
  ...
  setAiApiKeyOpenAI: (key: string) => void;
  aiApiKeyAnthropic: string;
  setAiApiKeyAnthropic: (key: string) => void;
  aiApiKeyGemini: string;
  setAiApiKeyGemini: (key: string) => void;
  aiSemanticSearchEnabled: boolean;
  setAiSemanticSearchEnabled: (enabled: boolean) => void;
...
export function SettingsModal({
  ...
  const providerMeta: Record<string, { label: string; placeholder: string }> = {
    openai: {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const models = await fetchModels(aiProvider as any, key, () => { });
      if (models.length > 0) {
            ...
            <div className="settings-danger-card">
              ...
              <div className="settings-action-row">
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      const p = await (window as any).api.getDataPath();
