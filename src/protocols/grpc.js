...
export const GrpcProtocol = {
  ...
  parseProtoSchema(protoContent) {
    ...
    const fieldRegex = /(?:repeated\s+|optional\s+|required\s+)?(\w+)\s+(\w+)\s*=\s*(\d+)/g;

    let serviceMatch;
    while ((serviceMatch = serviceRegex.exec(protoContent)) !== null) {
      const service = { name: serviceMatch[1], methods: [] };
    ...
    while ((messageMatch = messageRegex.exec(protoContent)) !== null) {
      const msg = { name: messageMatch[1], fields: [] };
   ...
   */
  generateSampleBody(messageName, messages) {
